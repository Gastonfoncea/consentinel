...
import {
  counterpartyRouteTrustSchema,
  eventSchema,
  phoneConfirmationProviderSchema,
  requestSchema,
  stepUpChallengeIdSchema,
  stepUpRejectSchema
} from "./schemas";
...
export function createPlatanusMcpServer(runtime: KernelRuntime = getSharedKernelRuntime()) {
  ...
  server.registerTool(
    ...
    {
      title: "Get Step-Up Challenge",
      description: "Fetch a pending step-up challenge and its current state for a phone/app handoff.",
      inputSchema: stepUpChallengeIdSchema.shape
    },
    async ({ challengeId }: { challengeId: string }) => {
      return jsonResponse({
        challenge: await getStepUpChallenge(runtime, challengeId)
      });
    }
  );
  ...
  server.registerTool(
    ...
    {
      title: "Confirm Phone Step-Up",
      description: "Record that the user verbally confirmed the phone call and can continue in the app.",
      inputSchema: phoneConfirmationProviderSchema.shape
    },
    async ({
      challengeId,
      provider
      ...
      return jsonResponse(await confirmPhoneStepUp(runtime, challengeId, provider ?? "manual"));
    }
  );
  ...
  server.registerTool(
    ...
    {
      title: "Reject Step-Up",
      description: "Reject a pending step-up challenge because the user denied the action or reported duress.",
      inputSchema: stepUpRejectSchema.shape
    },
    async ({
      challengeId,
      reason
      ...
      return jsonResponse(await rejectStepUp(runtime, challengeId, reason));
    }
  );
