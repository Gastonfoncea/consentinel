// Ashima Arts simplex noise 3D — used for organic vertex displacement.
// MIT licensed, public domain. Industry standard for shader noise.
...
void main() {
  ...
  // Multi-octave noise for organic, non-repetitive deformation
...
void main() {
  // Fresnel — strong glow on edges, faint in the center
  vec3 viewDir = normalize(-v_world_position);
  float fresnel = 1.0 - max(dot(v_normal, viewDir), 0.0);
  fresnel = pow(fresnel, 2.5);

  // Noise-driven gradient between the two state colors
